...
list_cmp()     { echo "$1" | sed -n 's/^List Comparisons = //p'; }
...
# assert: input must be ACCEPTED, output sorted, == sort -n, and vec==list count
expect_sorted() {
  local desc="$1"; shift
  ...
  vc=$(vec_cmp "$out"); lc=$(list_cmp "$out")

  if [ "$got" != "$want" ]; then
    ko "$desc" "not sorted correctly: got [$got] want [$want]"; return
  fi
  if [ "$vc" != "$lc" ]; then
    ko "$desc" "comparison mismatch: vector=$vc list=$lc (algorithms diverged!)"; return
  fi
  ok "$desc  ${DIM}(n=$# sorted, cmp=$vc, vec==list)${RST}"
}
...
expect_size() {
  ...
  if [ -z "$why" ]; then
    ok "n=$n ($ordering)  ${DIM}cmp=$vc${bound:+ / opt $bound}, vec==list${RST}$note"
  else
    ko "n=$n ($ordering)" "$why"
  fi
}
...
expect_error "leading plus sign"          +1 2 3
...
# =============================================================================
#  4. Many sizes x orderings  (correctness + vec==list + F-J bound)
#     includes Jacobsthal boundaries: 5,11,21,43 ... and n-1/n/n+1 around them
# =============================================================================
hdr "Sizes x orderings (sorted? + vec==list count + F-J max)"
